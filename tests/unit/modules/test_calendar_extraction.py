    ...
    def test_encoding(self):
        ...
        for calendar_feature in self.calendar_features:
            ...
            if calendar_feature == CalendarFeature.hour_sine:
                self.assertEqual(x[0], np.sin(np.pi * 0 / 23), "Expecting hour feature starting from 0.")
            if calendar_feature == CalendarFeature.day_sine:
                self.assertEqual(x[0], np.sin(np.pi * 0 / 31), "Expecting day feature starting from 0.")
                self.assertEqual(x[30], np.sin(np.pi * 30 / 31), "Expecting day feature starting from 0.")
                self.assertEqual(x[31], np.sin(np.pi * 0 / 3), "Expecting day feature starting from 0.")
            if calendar_feature == CalendarFeature.weekday_sine:
                self.assertEqual(x[0], np.sin(np.pi * 5 / 6), "Expecting weekday feature starting from 0.")
                self.assertEqual(x[1], np.sin(np.pi * 6 / 6), "Expecting weekday feature starting from 0.")
                self.assertEqual(x[2], np.sin(np.pi * 0 / 6), "Expecting weekday feature starting from 0.")
            if calendar_feature == CalendarFeature.month_sine:
                self.assertEqual(x[0], np.sin(np.pi * 0 / 11), "Expecting month feature starting from 0.")
                self.assertEqual(x[30], np.sin(np.pi * 0 / 11), "Expecting month feature starting from 0.")
                self.assertEqual(x[31], np.sin(np.pi * 1 / 11), "Expecting month feature starting from 0.")
            if calendar_feature == CalendarFeature.month_cos:
                self.assertEqual(x[0], np.cos(np.pi * 0 / 11), "Expecting month feature starting from 1.")
                self.assertEqual(x[30], np.cos(np.pi * 0 / 11), "Expecting month feature starting from 1.")
                self.assertEqual(x[31], np.cos(np.pi * 1 / 11), "Expecting month feature starting from 1.")
            if calendar_feature == CalendarFeature.hour_cos:
                self.assertEqual(x[0], np.cos(np.pi * 0 / 23), "Expecting hour feature starting from 1.")
            if calendar_feature == CalendarFeature.day_cos:
                self.assertEqual(x[0], np.cos(np.pi * 0 / 31), "Expecting day feature starting from 1.")
                self.assertEqual(x[30], np.cos(np.pi * 30 / 31), "Expecting day feature starting from 1.")
                self.assertEqual(x[31], np.cos(np.pi * 0 / 31), "Expecting day feature starting from 1.")
            if calendar_feature == CalendarFeature.weekday_cos:
                self.assertEqual(x[0], np.cos(np.pi * 5 / 6), "Expecting weekday feature starting from 1.")
                self.assertEqual(x[1], np.cos(np.pi * 6 / 6), "Expecting weekday feature starting from 1.")
                self.assertEqual(x[2], np.cos(np.pi * 0 / 6), "Expecting weekday feature starting from 1.")
