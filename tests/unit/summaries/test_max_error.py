...
class TestMaxErr(unittest.TestCase):

    def setUp(self) -> None:
        self.max_err = MaxErr(name="NAME")

    def tearDown(self) -> None:
        self.max_err = None

    def test_get_params(self):
        self.assertEqual(self.max_err.get_params(),
                         {'offset': 0})

    def test_set_params(self):
        self.max_err.set_params(offset=24)
        self.assertEqual(self.max_err.get_params(),
                         {'offset': 24})
    ...
    def test_transform(self):
        ...
        test_result = self.max_err.transform(file_manager=MagicMock(), y=test_data['testCol'], gt=test_data['testCol'],
                                             pred1=test_data['predictCol1'],
    ...
    def test_transform_without_predictions(self):
        ...
        with pytest.raises(InputNotAvailable) as e_info:
            self.max_err.transform(file_manager=MagicMock(), y=test_data['testCol'])
    ...
    def test_save(self, cloudpickle_mock, open_mock):
        ...
        mae = MaxErr(name="NAME", filter_method=filter_mock)
    ...
    def test_load(self, cloudpickle_mock, open_mock):
        ...
        mae = MaxErr.load(load_information)
